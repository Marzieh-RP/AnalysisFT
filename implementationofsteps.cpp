    ...
    for (int i=0;i<G.size();i++){
        ...
        for (int j=0;j<G[i].second.size();j++){
            if (assignments[i] != assignments[G[i].second[j].first]){
                ...
                //first insert a new node with one outgoing edge to G[i].second[j].first, its weight being the second/speed + latency it includes a KB to bits per second conversion as well.
                double weight = latency_links[assignments[i]][assignments[G[i].second[j].first]] + G[i].second[j].second/speed_links[assignments[i]][assignments[G[i].second[j].first]]*8*1024/1000;
...
speed_links[0][0] = 0;
speed_links[1][0] = 1000;
speed_links[0][1] = 1000;
speed_links[0][1] = 1000;

latency_links[0][0] = 20;
latency_links[1][0] = 20;
latency_links[0][1] = 20;
latency_links[1][1] = 20;
...
for (int i=0;i<GP.size();i++){
    cout<<GP[i].first<<endl;
    cout<<"edges for "<<i<<endl;
    for (int j=0;j<GP[i].second.size();j++)
        cout<<GP[i].second[j]<<endl;
}
//step 2
